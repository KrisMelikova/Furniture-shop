...
class OrderCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            self.object = form.save()  # сохраняем новую форму с информацией, которую передал нам пользователь
            if orderitems.is_valid():
                orderitems.instance = self.object
                orderitems.save()

        if self.object.get_total_cost() == 0:  # удаляем недозаполненные объекты
            self.object.delete()
...
'''
# варианты работы с сигналами вместо менеджеров
